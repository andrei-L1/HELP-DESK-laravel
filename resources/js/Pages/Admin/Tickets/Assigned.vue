...
const props = defineProps({
    ...
    filters: {
        type: Object,
        default: () => ({
            status: '',           // ← not forced anymore
            search: '',
        }),
    },
    statuses: {
        type: Array,
        default: () => [],      // ← hopefully passed from backend (Open, In Progress, Closed, ...)
    },
    view: {
        type: String,
        required: true,
...
const applyFilter = (key, value) => {
    router.get(
        route('admin.tickets.assigned'),     // ← important: use assigned route
        {
            ...props.filters,
            [key]: value || null,
            page: 1,
...
const resetFilters = () => {
    router.get(
        ...
        { 
            status: null,           // ← reset to show all statuses
            search: '',
        },
        {
            preserveState: false,
...
const viewTicket = (ticketId) => {
    // Replace with real route when you create the detail page
    // router.get(route('admin.tickets.show', ticketId));
...
<template>
    ...
    <AdminNavigation>
        <template #header-title>
            <h1 class="text-xl font-semibold text-gray-900">Assigned Tickets</h1>
        </template>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                <span
                                    ...
                                    :class="{
                                        ...
                                        'bg-green-100 text-green-800': ticket.priority?.toLowerCase() === 'low' || !ticket.priority,
                                    }"
                                >
                                    {{ ticket.priority || '—' }}
                                </span>
                        ...
                        <tr v-if="tickets.data.length === 0">
                            <td colspan="7" class="px-6 py-8 text-center text-sm text-gray-500">
                                No assigned tickets found with the current filters.
                            </td>
                        </tr>
                    </tbody>
                </table>
