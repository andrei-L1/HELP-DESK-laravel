...
class SlaBreachNotification extends Notification implements ShouldQueue
{
    ...
    public function via(object $notifiable): array
    {
        ...
        if ($notifiable->wantsNotification('sla_breach', 'mail')) {
            $channels[] = 'mail';
        }
        
        if ($notifiable->wantsNotification('sla_breach', 'database')) {
            $channels[] = 'database';
        }

        if ($notifiable->wantsNotification('sla_breach', 'broadcast')) {
            $channels[] = 'broadcast';
        }
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->error()
            ->subject('🚨 SLA BREACH: Ticket #' . $this->ticket->ticket_number)
            ->greeting('Hello ' . $notifiable->first_name . '!')
            ->line('The following ticket has BREACHED its SLA resolution limit.')
            ->line('**Ticket #:** ' . $this->ticket->ticket_number)
            ->line('**Subject:** ' . $this->ticket->subject)
            ->line('**Breached At:** ' . $dueTime)
            ->action('Take Immediate Action', $url)
            ->line('This event has been logged and escalated.');
    }
    ...
    public function toArray(object $notifiable): array
    {
        return [
            ...
            'message'        => 'SLA BREACH: Ticket #' . $this->ticket->ticket_number . ' has exceeded its limit.',
            'type'           => 'breach',
            'url'            => $this->getTicketUrl($notifiable),
        ];
    }
